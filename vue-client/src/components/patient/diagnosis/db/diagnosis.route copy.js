...
module.exports = (io) => {
  ...
  router.get('/', async (req, res) => {
    try {
      ...
      const diagnoses = await Diagnosis.sequelize.query('SELECT *,ROW_START, ROW_END FROM diagnosis FOR SYSTEM_TIME ALL where patientUUId = :patientUUId order by ROW_START desc',
        {
      ...
      const promises = diagnoses.map(async (diagnosis, index) => {
        const { uuid, diagnosisName, diagnosedOnDate, recordChangedByUUID, recordChangedOnDateTime, ROW_START, ROW_END } = diagnosis
        ...
        try {
          ...
          const assessments = await DiagnosisAssessment.sequelize.query('SELECT *,ROW_START, ROW_END FROM diagnosisAssessment FOR SYSTEM_TIME ALL where diagnosisUUID = :uuid order by ROW_START desc',
            {
          ...
          const assessmentPromises = assessments.map(async (history, innerIndex) => {
            const { diagnosisAssessment, recordChangedByUUID, ROW_START, ROW_END } = history
      ...
      const diagnosesEvalAtEachRowEnd = await Promise.all(promises)
      let resultToReturn = {};

      diagnosesEvalAtEachRowEnd.forEach(element => {
        const properyUUID = element.uuid
        if (typeof resultToReturn[properyUUID] == 'undefined') {
          resultToReturn[properyUUID] = []
        }

        const propertyRowEnd = element.ROW_END
        const propertyRowStart = element.ROW_START
        let eachDxObj = {};
        eachDxObj[propertyRowEnd] = {}

        eachDxObj[propertyRowEnd][propertyRowStart] = element
        resultToReturn[properyUUID].push(eachDxObj);

        //var rowEnd = element.ROW_END
        /*var resultWithProperStructure = {};
        if (typeof resultWithProperStructure[`${element.uuid}`] == `undefined`) {
          resultWithProperStructure[`${element.uuid}`] = { data: element, key: `${element.uuid}` }
        }*/
        //resultWithProperStructure[`${element.uuid}`].push()
        //resultToReturn.push(resultWithProperStructure);
        //resultToReturn = { "${element.uuid}": element }


        //resultToReturn[element.uuid][element.ROW_END] = element
        // resultToReturn.push(element);
      });

      //res.send(diagnosesEvalAtEachRowEnd)
