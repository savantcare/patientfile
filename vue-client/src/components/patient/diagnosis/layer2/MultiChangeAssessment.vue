    ...
    <el-row :gutter="12">
        ...
        :autoplay="false">
            ...
            :key="`dxCarousel-${index}`">
                <el-col 
                :span="8" 
                v-for="(dx, index) in dxList" 
                :key="index">
                    <el-card  class="box-card" shadow="hover" style="font-family: Helvetica;">
                        <div><b>Name:</b> {{dx.diagnosisName}}</div>
                        <div><b>Diagnosed On:</b> {{dx.diagnosedOnDate}}</div>
                        <div><b>Added On:</b>{{dx.diagnosisAddedDetails}}</div>
                        ...
                        <el-form-item>
                            ...
                            <el-button type="danger" @click="onClickDiscontinue(dx)" size="small">Discontinue</el-button>
                        </el-form-item>
                        </el-form>
                        <DiagnosisAssessmentHistoryItem :dx="dx"></DiagnosisAssessmentHistoryItem>
                    </el-card> 
                </el-col>
            </el-carousel-item>
        </el-carousel>
...
import DiagnosisAssessmentHistoryItem from "./DiagnosisAssessmentHistoryItem";
export default {
  components: {
    DiagnosisAssessmentHistoryItem
  },
  data() {
    return {
    };
  ...
  computed: {
    dxList() {
      return this.$store.state.diagnosis.diagnosisList;
    },
    sliderSet() {
      let result = [];
      let temp = [];
