...
export default {
  ...
  mutations: {
     ...
     * Socket Listeners
     */
    SOCKET_ON_UPDATE_DIAGNOSES(state, updateList) {
      // Message received from socket server
      state.diagnosisList = updateList;
    },
    SOCKET_ADD_DIAGNOSIS(state, newData) {
      // Msg recd from node-server/routes/diagnosis.route.js
      if (state.diagnosisList.length > 0) {
        // At the client where this data was added it needs to be skipped
  ...
  actions: {
    ...
    async changeDiagnosisAssessment({ state, commit }, json) {
      ...
      try {
        const response = await fetch(
          `${DIAGNOSIS_API_URL}/changeAssessment/${data.uuid}`,
          {
            method: "PUT",
            headers: {
              "Content-Type": "application/json;charset=utf-8",
    ...
    async discontinueDiagnosisAssessment({ state, commit }, json) {
      ...
      try {
        data["discontinue"] = true;
        const response = await fetch(
          `${DIAGNOSIS_API_URL}/discontinueAssessment/${assessmentUUID}`,
          {
            method: "PATCH",
            headers: {
              "Content-Type": "application/json;charset=utf-8",
