  ...
  methods: {
    async submitForm(element) {
      ...
      const data = {
        ...
        temperatureInFarehnite: element.value
      };

      await this.$store.dispatch("bodyMeasurement/dbUpdateTemperatureInSM", {
        data: data,
        notify: this.$notify
      });
    ...
    getChangeLog() {
      ...
      const currentValue = Temperature.query()
        .where("ROW_END", value => value > currentUnixTimeStamp)
        .get();
      if (currentValue.length > 0) {
        this.currentValue = {
          value: currentValue[0].temperatureInFarehnite,
          date: currentValue[0].timeOfEvaluation,
          notes: currentValue[0].Notes
        };
      }
      const changeLog = Temperature.query()
        .where("ROW_END", value => value < currentUnixTimeStamp)
        .get();
      if (changeLog.length > 0) {
        ...
        changeLog.forEach(log => {
          this.changeLog.push({
            ...
            value: log.temperatureInFarehnite
          });
        });
      }
    }
