  ...
  methods: {
    async submitForm(element) {
      ...
      const data = {
        ...
        beatsPerMinuteValue: element.value
      };

      await this.$store.dispatch("bodyMeasurement/dbUpdatePulseInSM", {
        data: data,
        notify: this.$notify
      });
