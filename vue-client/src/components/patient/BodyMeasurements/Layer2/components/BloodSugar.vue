  ...
  methods: {
    async submitForm(element) {
      ...
      const data = {
        ...
        bloodSugar: element.value
      };

      await this.$store.dispatch("bodyMeasurement/dbUpdateBloodSugarInSM", {
        data: data,
        notify: this.$notify
      });
