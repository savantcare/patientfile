  ...
  methods: {
    async submitForm(element) {
      ...
      const data = {
        ...
        waistCircumferenceInInches: element.value
      };

      await this.$store.dispatch(
        "bodyMeasurement/dbUpdateWaistCircumferenceInSM",
        {
          data: data,
          notify: this.$notify
        }
    ...
    getChangeLog() {
      ...
      const currentValue = WaistCircumference.query()
        .where("ROW_END", value => value > currentUnixTimeStamp)
        .get();
      if (currentValue.length > 0) {
        this.currentValue = {
          value: currentValue[0].waistCircumferenceInInches,
          date: currentValue[0].timeOfEvaluation,
          notes: currentValue[0].Notes
        };
      }
      const changeLog = WaistCircumference.query()
        .where("ROW_END", value => value < currentUnixTimeStamp)
        .get();
      if (changeLog.length > 0) {
        ...
        changeLog.forEach(log => {
          this.changeLog.push({
            ...
            value: log.waistCircumferenceInInches
          });
        });
      }
    }
